...
// creates NAPI type ObjectResult for returning it to javascript
napi_value createObjectResult(napi_env env,
  Object* objectPtr, Error* error_result) {
  napi_value returnObject, objectNAPI, errorObject,
  ...
  if (error_result != NULL) {
    ...
    status = napi_create_string_utf8(env,
      errorResult.message, NAPI_AUTO_LENGTH, &errorMessage);
    assert(status == napi_ok);

    status = napi_set_named_property(env, errorObject, "code", errorCode);
    assert(status == napi_ok);

    status = napi_set_named_property(env, errorObject, "message",
      errorMessage);
    assert(status == napi_ok);
    ...
    status = napi_create_string_utf8(env,
      &empty[0], NAPI_AUTO_LENGTH, &nullvalue);
    assert(status == napi_ok);

    status = napi_set_named_property(env, returnObject, "object",
      nullvalue);
    assert(status == napi_ok);

    status = napi_set_named_property(env, returnObject, "error",
      errorObject);
    assert(status == napi_ok);
    ...
    status = napi_set_named_property(env,
      returnObject, "object", objectNAPI);
    assert(status == napi_ok);
    ...
    status = napi_set_named_property(env,
      errorObject, "message", nullvalue);
    assert(status == napi_ok);

    status = napi_set_named_property(env,
      returnObject, "error", errorObject);
    assert(status == napi_ok);
    free_object(objectPtr);
  }
  return returnObject;
}
...
napi_value createResult(napi_env env, string resultType,
  ...
  status = napi_set_named_property(env,
    returnObject, &resultTypeChar[0], ObjectRef);
  assert(status == napi_ok);
...
napi_value parse_accessc(napi_env env, napi_callback_info info) {
  ...
  //  * requests satellite for a new access grant using a passhprase
  //  * pre-requisites: None
  //  * inputs: satellite address (String), API key (String)
  //  passphrase (String)
  //  * output: AccessResult (Object)
napi_value request_access_with_passphrasec(napi_env env,
  ...
  request_access_with_passphrase(satellite_address, api_key, passphrase);
  ...
  //  * requests satellite for a new access grant using a passhprase and config
  //  * pre-requisites: None
  //  * inputs: satellite address (String), API key (String)
  //  * passphrase (String)
  //  * output: AccessResult (Object)
napi_value config_request_access_with_passphrasec(napi_env env,
  ...
  AccessResult access_Result = config_request_access_with_passphrase(
    config, satellite_address, api_key, passphrase);
  delete[] satellite_address;
  delete[] api_key;
  delete[] passphrase;
  ...
  if (access_Result.access == NULL) {
    if (access_Result.error != NULL) {
      Error error_result = *(access_Result.error);
      char* errorMessagePtr = error_result.message;
      return createResult(env,
        "access", 0, error_result.code, &errorMessagePtr[0]);
    } else {
      char blank[] = "";
      return createResult(env, "access", 0, -1, &blank[0]);
    }
...
napi_value open_projectc(napi_env env, napi_callback_info info) {
  ...
  //  * opens project using access grant and config.
  //  * pre-requisites: request_access_with_passphrasec
  //  * inputs: Access (Object)
  //  * output: ProjectResult (Object)
napi_value config_open_projectc(napi_env env, napi_callback_info info) {
  ...
  if (project_result.project == NULL) {
    if (project_result.error != NULL) {
      Error error_result = *(project_result.error);
      char* errorMessagePtr = error_result.message;
      return createResult(env, "project", 0, error_result.code,
        errorMessagePtr);
    } else {
      char blank[] = "";
      return createResult(env, "project", 0, -1, &blank[0]);
...
napi_value create_bucketc(napi_env env, napi_callback_info info) {
  ...
  char* bucketName =  new char[convertedvalue];
  status = napi_get_value_string_utf8(env,
    args[1], bucketName, convertedvalue, &bufsize);
  assert(status == napi_ok);
...
napi_value ensure_bucketc(napi_env env, napi_callback_info info) {
  ...
  convertedvalue = convertedvalue+1;
  char* bucketName =  new char[convertedvalue];
  status = napi_get_value_string_utf8(env, args[1], &bucketName[0],
    convertedvalue, &bufsize);
  assert(status == napi_ok);
...
napi_value list_bucketsc(napi_env env, napi_callback_info info) {
  ...
  if (checktypeofinput == napi_null) {
      ...
      bucket_resultIterator = list_buckets(&project_result,
        &listBucketsOptions);
      delete[] cursor;
    }
  ...
  status = napi_set_named_property(env, BucketInfoObj, "name",
    bucketName);
  assert(status == napi_ok);

  napi_value createdBucket;
  int64_t bucketcreatedvalue = bucket_result->created;
  status = napi_create_int64(env, bucketcreatedvalue,
    &createdBucket);
  assert(status == napi_ok);
  status = napi_set_named_property(env, BucketInfoObj, "created",
    createdBucket);
  assert(status == napi_ok);
  char str[100];
  itoa(count, str, 10);
...
if (err == NULL) {
  ...
  status = napi_create_string_utf8(env, &message[0], NAPI_AUTO_LENGTH,
    &errormessage);
  assert(status == napi_ok);

  status = napi_set_named_property(env, errorObject, "error",
    codenapi);
  assert(status == napi_ok);
  ...
  assert(status == napi_ok);
  status = napi_create_string_utf8(env, &messagePtr[0], NAPI_AUTO_LENGTH,
    &errormessage);
  assert(status == napi_ok);
...
assert(status == napi_ok);
//
free_bucket_iterator(bucket_resultIterator);
return returnObject;
}
...
napi_value stat_objectc(napi_env env, napi_callback_info info) {
  ...
  ObjectResult object_result = stat_object(&project_result, bucketName,
    objectKey);
  delete[] bucketName;
  delete[] objectKey;
  ...
  //  * pre-requisites: open_projectc
  //  * inputs: Project (Object), Bucket Name (String)
  //  * Object Key(String), Uplaod Options(Object)
...
napi_value upload_objectc(napi_env env, napi_callback_info info) {
  ...
  UploadResult upload_result;
      if (checktypeofinput == napi_null) {
        upload_result = upload_object(&project_result, &bucketName[0],
          &objectKey[0], NULL);
      } else {
        ...
        upload_result = upload_object(&project_result, bucketName,
          objectKey, &uploadOptions);
      }
      delete[] bucketName;
      delete[] objectKey;
      if (upload_result.upload == NULL) {
        if (upload_result.error != NULL) {
          ...
          char* errorMessagePtr = error_result.message;
          return createResult(env, "upload", 0, error_result.code,
            &errorMessagePtr[0]);
        } else {
          char blank[] = "";
  ...
  //  * function upload_writec uploads len(p)bytes from p to the object's
  //  * data stream.
...
napi_value upload_writec(napi_env env, napi_callback_info info) {
  ...
  WriteResult write_result = upload_write(&upload_resultRef,
    ptrToData, bytesread);
      ...
      if (write_result.error != NULL) {
          ...
          assert(status == napi_ok);
          status = napi_set_named_property(env, errorObject, "code",
            errorCode);
          assert(status == napi_ok);
  ...
  //  * pre-requisites: open_projectc
  //  * inputs: Project (Object), Buxcket Name(String)
  //  * Object Key(String), Download Options(Object)
...
napi_value download_objectc(napi_env env, napi_callback_info info) {
  ...
  DownloadResult download_result;
      if (checktypeofinput == napi_null) {
        download_result = download_object(&project_result, bucketName,
          objectKey, NULL);
      } else {
...
// * function download_readc downloads from object's
// * data stream into bytes up to length amount.
// * pre-requisites: download_objectc
...
//  * creates new access grant with specific permission.
//  * Permission will be applied to prefixes
//  * pre-requisites: parse_accessc
//  * inputs: Access (Object), Permission (Object)
//  * Share Prefix (Object), prefix count (int)
...
napi_value access_sharec(napi_env env, napi_callback_info info) {
  ...
  for (uint32_t i=0; i < (uint32_t)sizeOfArrayInt; i++) {
    ...
    *(SharePrefixListPointer+i) = sharePrefix;
      ...
      AccessResult access_Result = access_share(&access, permission,
        SharePrefixListPointer, sharePrefixSize);
...
napi_value list_objectsc(napi_env env, napi_callback_info info) {
  ...
  if (checktypeofinput == napi_null) {
        ...
        objectIterator = list_objects(&project_result, bucketName,
          &listObjectsOptions);
      ...
      int count = 0;
              ...
              while (object_iterator_next(objectIterator)) {
                ...
                if (customMetadata.count > 0) {
                  ...
                  status = napi_set_named_property(env, entriesArray,
                    "0", emptyNAPI);
                  assert(status == napi_ok);
                }
                napi_value createdNAPI;
            ...
            if (err == NULL) {
              ...
              assert(status == napi_ok);
                ...
                status = napi_create_int64(env, errorResult.code,
                  &codenapi);
                assert(status == napi_ok);

                status = napi_create_string_utf8(env, &messagePtr[0],
                  NAPI_AUTO_LENGTH, &errormessage);
                assert(status == napi_ok);
...
napi_value access_serializec(napi_env env, napi_callback_info info) {
  ...
  StringResult string_result = access_serialize(&access);
      ...
      if (string_result.error != NULL) {
        ...
        status = napi_create_string_utf8(env,
          StringResultPtr, NAPI_AUTO_LENGTH, &stringNAPI);
        assert(status == napi_ok);
      }
      ...
      status = napi_set_named_property(env, returnObject,
        "string", stringNAPI);
      assert(status == napi_ok);
      free_string_result(string_result);
      return returnObject;
}


//  * function upload_info returns information about the uploaded object.
...
napi_value Init(napi_env env, napi_value exports) {
  ...
  napi_property_descriptor  open_project = DECLARE_NAPI_METHOD(
    "open_project", open_projectc);
  status = napi_define_properties(
    env, exports, 1, & open_project);
  assert(status == napi_ok);

  napi_property_descriptor  config_open_project = DECLARE_NAPI_METHOD(
    "config_open_project", config_open_projectc);
  status = napi_define_properties(
    env, exports, 1, & config_open_project);
  assert(status == napi_ok);

  napi_property_descriptor close_project = DECLARE_NAPI_METHOD(
    "close_project", close_projectc);
  status = napi_define_properties(
    env, exports, 1, &close_project);
  assert(status == napi_ok);

  napi_property_descriptor stat_bucket = DECLARE_NAPI_METHOD(
    "stat_bucket", stat_bucketc);
  status = napi_define_properties(
    env, exports, 1, &stat_bucket);
  assert(status == napi_ok);

  napi_property_descriptor create_bucket = DECLARE_NAPI_METHOD(
    "create_bucket", create_bucketc);
  status = napi_define_properties(
    env, exports, 1, &create_bucket);
  assert(status == napi_ok);

  napi_property_descriptor ensure_bucket = DECLARE_NAPI_METHOD(
    "ensure_bucket", ensure_bucketc);
  status = napi_define_properties(
    env, exports, 1, &ensure_bucket);
  assert(status == napi_ok);

  napi_property_descriptor delete_bucket = DECLARE_NAPI_METHOD(
    "delete_bucket", delete_bucketc);
  status = napi_define_properties(
    env, exports, 1, &delete_bucket);
  assert(status == napi_ok);

  napi_property_descriptor list_buckets = DECLARE_NAPI_METHOD(
    "list_buckets", list_bucketsc);
  status = napi_define_properties(
    env, exports, 1, &list_buckets);
  assert(status == napi_ok);

  napi_property_descriptor stat_object = DECLARE_NAPI_METHOD(
    "stat_object", stat_objectc);
  status = napi_define_properties(
    env, exports, 1, &stat_object);
  assert(status == napi_ok);

  napi_property_descriptor delete_object = DECLARE_NAPI_METHOD(
    "delete_object", delete_objectc);
  status = napi_define_properties(
    env, exports, 1, &delete_object);
  assert(status == napi_ok);

  napi_property_descriptor upload_object = DECLARE_NAPI_METHOD(
    "upload_object", upload_objectc);
  status = napi_define_properties(
    env, exports, 1, &upload_object);
  assert(status == napi_ok);

  napi_property_descriptor upload_write = DECLARE_NAPI_METHOD(
    "upload_write", upload_writec);
  status = napi_define_properties(
    env, exports, 1, &upload_write);
  assert(status == napi_ok);

  napi_property_descriptor upload_commit = DECLARE_NAPI_METHOD(
    "upload_commit", upload_commitc);
  status = napi_define_properties(
    env, exports, 1, &upload_commit);
  assert(status == napi_ok);

  napi_property_descriptor download_object = DECLARE_NAPI_METHOD(
    "download_object", download_objectc);
  status = napi_define_properties(
    env, exports, 1, &download_object);
  assert(status == napi_ok);

  napi_property_descriptor download_read = DECLARE_NAPI_METHOD(
    "download_read", download_readc);
  status = napi_define_properties(
    env, exports, 1, &download_read);
  assert(status == napi_ok);

  napi_property_descriptor close_download = DECLARE_NAPI_METHOD(
    "close_download", close_downloadc);
  status = napi_define_properties(
    env, exports, 1, &close_download);
  assert(status == napi_ok);

  napi_property_descriptor list_objects = DECLARE_NAPI_METHOD(
    "list_objects", list_objectsc);
  status = napi_define_properties(
    env, exports, 1, &list_objects);
  assert(status == napi_ok);
  ...
  napi_property_descriptor access_serialize = DECLARE_NAPI_METHOD(
    "access_serialize", access_serializec);
  status = napi_define_properties(env, exports, 1, &access_serialize);
  assert(status == napi_ok);

  napi_property_descriptor download_info = DECLARE_NAPI_METHOD(
    "download_info", download_infoc);
  status = napi_define_properties(env, exports, 1, &download_info);
  assert(status == napi_ok);

  napi_property_descriptor upload_info = DECLARE_NAPI_METHOD(
    "upload_info", upload_infoc);
  status = napi_define_properties(env, exports, 1, &upload_info);
  assert(status == napi_ok);

  napi_property_descriptor upload_abort = DECLARE_NAPI_METHOD(
    "upload_abort", upload_abortc);
  status = napi_define_properties(env, exports, 1, &upload_abort);
  assert(status == napi_ok);

  napi_property_descriptor upload_set_custom_metadata = DECLARE_NAPI_METHOD(
    "upload_set_custom_metadata", upload_set_custom_metadatac);
